...
import { InputText, Message, RadioButton, Select, ToggleSwitch } from 'primevue'
...
const targetSettings = defineModel('targetSettings') as Ref<
  targets.TargetSettings & { settings: targets.nzbget.Settings }
>
...
<template>
  ...
  <div class="flex items-center gap-4 mb-4">
    ...
    >
      <div class="flex items-center">
        <ToggleSwitch v-model="targetSettings.settings.addPaused as boolean" />
  ...
  <div class="flex items-center gap-4 mb-4">
    <label for="name" class="font-semibold w-24 text-right">{{ i18n.t('common.settings.dupeMode.title') }}</label>
    <FormField
      :name="i18n.t('common.settings.dupeMode.title')"
      :initial-value="targetSettings.settings.dupeMode"
      class="grid-row flex-auto"
    >
      <div class="flex items-center gap-4">
        <div class="flex items-center gap-2">
          <RadioButton v-model="targetSettings.settings.dupeMode" input-id="Force" name="dupeMode" value="Force" />
          <label for="Force">Force</label>
        </div>
        <div class="flex items-center gap-2">
          <RadioButton v-model="targetSettings.settings.dupeMode" input-id="Score" name="dupeMode" value="Score" />
          <label for="Score">Score</label>
        </div>
        <div class="flex items-center gap-2">
          <RadioButton v-model="targetSettings.settings.dupeMode" input-id="All" name="dupeMode" value="All" />
