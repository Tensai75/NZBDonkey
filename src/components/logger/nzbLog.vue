...
const loadLogsLazy = async (event: { first: number; last: number }, showLoader: boolean = true) => {
  try {
    ...
    last.value = event.last > 0 ? event.last : 100 // Ensure last is at least 100 if not provided
    ...
    console.log(`Loading logs from ${first.value} to ${last.value}`)
    const loadedLogs = await nzbLogger.getLazy(first.value, last.value)
    console.log(`Loaded ${loadedLogs.length} logs`)

    //populate page of virtual logs
...
const confirmDelete = (): void => {
  confirm.require({
    ...
    accept: async () => {
      nzbLogger.clear()
      loadLogsLazy({ first: 0, last: 100 }) // Reload the logs after clearing
    },
    reject: () => {
      // void
    },
...
async function loadTotalCount() {
  ...
  if (count !== totalCount.value) {
    ...
    console.log(`Total count updated: ${totalCount.value}`)
    return true
  } else {
    return false
  }
...
<template>
  <DataTable
    ...
    footer-style="padding: var(--p-datatable-footer-padding) 0"
